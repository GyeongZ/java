...
<!-- 
	...
	- STS 콘솔창에 화면에서 보낸 ID가 출력
		1. 화면구성(링크추가)
			- 보낼 URL을 선정 : /id (c:url 사용 => 컨스트럭처?)
			- 보낼 방식 : get
			- 보낼 데이터의 변수명 지정(name) : id
...
<!-- 
	a태그를 이용하여 서버에 이름을 전송해서 STS 콘솔창에 출력하는 코드를 작성하세요.
		...
		2. 컨트롤러에 메서드 확인
 -->
 <a href="<c:url value="/name?name=홍길동"/>">홍길동 보내기</a>
 
 <!-- 
 	LoginDTO 클래스를 이용하여 화면에서 id와 pw를 서버에 전송하고,
 	서버에서는 화면에서 보낸 id와 pw를 콘솔창에 출력하는 코드를 작성하세요.
 	0. LoginDTO 추가
 		- id와 pw
 		- 작업의 편리를 위해 lombok 연결
	1. 링크 추가 (비밀번호 노출되면 X => post 방식) (!! name 변수는 DTO나 VO와 같도록 해야함)
		- URL : /login
	2. 컨트롤러 메서드 확인
		- 콘솔창에 id와 pw가 담긴 LoginDTO 객체를 출력한 후 test.jsp로 연결
  -->
