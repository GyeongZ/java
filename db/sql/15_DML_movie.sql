# 사용자가 아이디 abc123, 비번 asd123으로 회원가입을 진행 (필수 들어가야할거 3가지 다 적는 방식으로 적음)
insert into member value('abc123', 'asd123', 'USER');	
# 사용자가 아이디 qwe123, 비번 qweqwe으로 회원가입을 진행 (앞에 (me_id, me_pw) 라고 적었기 때문에 뒤에 null 생략)
insert into member(me_id, me_pw) value('qwe123', 'qweqwe');	
# 관리자가 아이디 admin, 비번 admin 으로 회원 가입을 진행
...
# 1. 폴킹, 티모시 살라메, 칼라 레인, 올리비아 콜맨, 톰 데이비스, 휴 그랜트, 샐리 호킨 정보를 charater 테이블에 추가
insert into `character` values(null, '폴킹', '1978-07-29', '','미국'),
(null, '티모시 살라메', '1995-12-27', '','미국'),
...
# 2. 폴킹, 티모시 살라메, 칼라 레인, 올리비아 콜맨, 톰 데이비스, 휴 그랜트, 샐리 호킨 정보를 movie_person 테이블에 추가(1에서 추가한 정보를 이용)
insert into movie_person(mp_role, mp_pic, mp_ch_num) values('감독','',1),
('배우','',2),
('배우','',3),
('배우','',4),
('배우','',5),
...
insert into age values ('전체관람가'), ('12세 이상 관람가'), ('15세 이상 관람가'),('청소년 관람불가'), ('제한 관람가');
...
# 5. 폴킹, 티모시 살라메, 칼라 레인, 올리비아 콜맨, 톰 데이비스, 휴 그랜트, 샐리 호킨 정보를 join 테이블에 추가 (2,3에서 추가한 정보를 이용) 배역을 모르면 ""로
...
insert into production_nation(pn_mo_num, pn_na_name) values(1,'미국'),(2,'영국');

# 10. 장르 포함에 영화와 장르를 추가
insert into genre_include(gi_ge_name, gi_mo_num) values('환타지',1),('드라마',2) ;

# 11. 지역을 추가하는 쿼리(서울, 경기, 인천, 강원, 대전/충청, 대구, 부산/울산, 경상, 광주/전라/제주)
insert into region values('서울'),('경기'),('인천'),('강원'),('대전/충청'),('대구'),('부산/울산'),('경상'),('광주/전라/제주');

# 영화관을 추가하는 쿼리
# CGV강남, 서울특별시 강남구 역삼동, 총 좌석수 : 0, 총 상영관 :3, 지역 : 서울
# CGV영등포, 서울특별시 영등포구 4가, 총 좌석수 : 0, 총 상영관 :4, 지역 : 서울
...
insert into screen values (null, 1,10,1);
insert into seat(se_name, se_sc_num) values
('A1',1),('A2',1),('A3',1),
('B1',1),('B2',1),('B3',1),
('C1',1),('C2',1),('C3',1),('C4',1);

insert into screen values (null,2,12,1);
insert into seat(se_name, se_sc_num) values
('A1',2),('A2',2),('A3',2),('A4',2),
('B1',2),('B2',2),('B3',2),('B4',2),
('C1',2),('C2',2),('C3',2),('C4',2);

insert into screen values (null,3,6,1);
insert into seat(se_name, se_sc_num) values
('A1',3),('A2',3),
('B1',3),('B2',3),
('C1',3),('C2',3);

# 극장 좌석 수정 (정보가 정확하던 아니던 UPDATE 한 숫자로 바로 바뀜)
update theater
set
	th_seat = 11111
where
...
# 극장 좌석 정보를 받아와서 정확한 숫자 수정 (서브쿼리, 쿼리 안에 쿼리)
/* 서브 쿼리를 이용하여 CGV강남에 등록된 좌석수를 계산하여 영화관 전체 좌석수에 업데이트하는 쿼리
 	- 서브 쿼리로 A테이블을 업데이트 할 때, 서브 쿼리에 A테이블을 단순 이용하면 업데이트가 되지 않음.
 	- 이럴 때, A테이블이 아닌 A테이블을 조회한 결과를 이용하면 가능
	- 서브쿼리가 SELECT COUNT(*) FROM A 일때, A대신 (select * from A) as 임시이름 을 이용해야한다.
*/
update theater
set
	th_seat = (select count(*)
from seat
		join
...
select* from member;
# abc123 회원이 웡카를 예매했을 때 쿼리 (CGV강남 1상영관 10:30 영화를 성인 2명, A1, A2를 예매)
# 2가지 작업해야함

# ticketing 테이블에 데이터 추가
insert into ticketing values (null,2,0, 11200*2,1,'abc123');
# ticketing_list 테이블에 데이터 추가
insert into ticketing_list(tl_ti_num,tl_se_num) values (1,1),(1,2);
...
# 폴킹 감독이 웡카 영화(영화번호 1)에 감독으로 참여하는 쿼리
INSERT INTO `join`
SELECT 1, '감독', mp_num , 1
FROM
	movie_person
		JOIN
