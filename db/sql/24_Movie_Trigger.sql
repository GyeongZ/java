...
# 상영관에 좌석이 추가되면 상영관 전체 좌석수와 전체 좌석수를 업데이트하는 트리거
...
BEGIN
    ...
    # 영화관 전체 좌석 수
    SET _TH_SEAT = (SELECT COUNT(*) FROM SEAT WHERE SE_SC_NUM IN
					(SELECT SC_NUM FROM SCREEN WHERE SC_TH_NUM = _TH_NUM));
...
# 상영관이 추가되면 영화관의 전체 상영관 수를 업데이트하는 트리거
