...
*사용자 기능 (회원 + 비회원)
	- 조회(무비차트)
		...
		- 영화는 오늘 이후에 일정들을 예매한 내역을 기준으로 조회 한다.
        - 예매 내역이 많은 순으로 조회한다.
    ...
    - 조회(상영시간)
        ...
        - 사용자가 영화, 극장, 날짜를 선택하면 극장 정보(극장 이름, 전체 좌석수)와 상영시간 별 현재 예매 가능 좌석수를 조회할 수 있다.
    
    - 조회(영화 상세 조회)
		- 사용자는 영화 상세 정보를 조회할 수 있다.
        - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다.
        - 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다.
        - 기본정보에는 관람 나이정보, 런닝 시간(분 단위), 제작 국가가 있다.
...
*관리자 기능
    ...
    - 영화 관리
        ...
        - 기본정보에서 연령은 등록된 연력 중 하나를 선택할 수 있다.
        - 런닝 시간은 분으로 입력한다.
    ...
    - 상영관 관리
        ...
        - 상영관 번호는 극장의 총 관수보다 작거나 같아야하며, 같은 극장에 등록된 상영관 번호는 쓸 수 없다.
