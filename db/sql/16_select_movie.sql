...
where sh_date between "2024-02-09"and "2024-02-10";

# 서브쿼리를 사용한 쿼리 (join 전에 조회를 하기 때문에 처리가 더 빨라짐)
select distinct
	mo_title, sh_date
from
	(select * from schedule
    where sh_date between "2024-02-09"and "2024-02-10") as sh
join
	movie on sh_mo_num = mo_num;
...
-- 	theater
-- set
-- 	th_seat = (영화관 번호가 1번인 영화관에 등록된 상영관 좌석들의 합),
--     th_screen = (영화관 번호 1번인 영화관에 등록된 상영관 수)
-- where
-- 	th_num = 1;
