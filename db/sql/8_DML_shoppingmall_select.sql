...
SELECT
	ca_name, IF(count(PR_CA_NUM) > 0, COUNT(PR_CA_NUM), '등록된 제품 없음') AS '카레고리별 제품 수'
FROM
	product
		RIGHT JOIN
	category ON PR_CA_NUM = CA_NUM
...
SELECT
	MEMBER.*, SUM(OR_TOTAL_PRICE)
FROM
	`ORDER`
		RIGHT JOIN
	MEMBER ON OR_ME_ID = ME_ID
GROUP BY ME_ID;
# 이때 구매 안한 회원은 NULL로 나오므로 0원으로 맞춰주려고 한다.
SELECT
	ME_ID AS '아이디', IFNULL(SUM(OR_TOTAL_PRICE),'0원')
FROM
	`ORDER`
		RIGHT JOIN
	MEMBER ON OR_ME_ID = ME_ID
...
# 회원 별 등급을 조회하는 쿼리.
# 등급은 기본이 브론즈, 누적금액이 5만원 이상이면 실버, 8만원 이상이면 골드
SELECT
	ME_ID AS '아이디',
    CASE
    WHEN IFNULL(SUM(OR_TOTAL_PRICE),'0원') >= 8000 THEN '골드'
    WHEN IFNULL(SUM(OR_TOTAL_PRICE),'0원') >= 5000 THEN '실버'
    ELSE '브론즈'
    END AS '등급'
FROM
	`ORDER`
