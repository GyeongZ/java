...
public class StudentProgram {
	...
	public static void main(String[] args) {
		...
		// while(선택한 메뉴 != 프로그램 종료);
		scan.close();
	}
	
	

	// **********메서드 만들기 (main 메서드 밖에서)
	...
	public static void printScore(Student []stds) {
		...
		//서브 메뉴가 1이면 과목별 조회
		case 1:
				//학생 배열을 주고 조회할 과목을 입력받아 과목에 맞는 성적을 출력하라고 시킵니다.(메서드)
			printScoreBySubject(stds);
			break;
			
		//서브 메뉴가 2이면 학생별 조회
		case 2:	
				//학생 배열을 주고 학생 번호를 입력받아 번호에 맞는 학생 성적을 출력하라고 시킵니다.(메서드)
	...
	public static void printScoreByNum(Student [] stds) {
		...
		//없는 학생 번호이면 없는 학생이라고 출력 (여기까지 온 것은 학생 정보가 없다는 것)
