...
public class ClassCastingEx1 {
	...
	public static void main(String[] args) {
		...
		//print()는 부모 클래스에도 있는 기능이기 때문에 호출 가능
		p1.print();
		//print2()는 자식 클래스에서만 있는 기능이기 때문에 부모 인스턴스가 호출 할 수 없다.
	
		
		/* 다운 캐스팅 : 명시적 클래스 형변환
		 클래스 형변환을 반드시 표기
		 예외 발생할수있다 : 안되는 경우
		 부모 클래스 인스턴스를 자식 클래스 인스턴스에 저장하려고 할 때
		Child c2 = (Child) new Parent(1);
		c2.print(); 이건 안됨
		
		*다운 캐스팅이 되려면
		*1. 업캐스팅으로 부모 인스턴스에 자식 인스턴스가 저장되어야함
		*2. 업캐스팅 된 부모 인스턴스를 자식 인스턴스에 저장되어야함
		*/
		//업 캐스팅
		...
		//다운캐스팅 (일시적으로)
		//다운캐스팅을 통해 부모 인스턴스로 자식 인스턴스의 메서드인 print2를 호출했음.
		((Child)p4).print2();
	}
