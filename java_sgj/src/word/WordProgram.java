...
public class WordProgram implements Program {
	...
	public void run() {
		...
		do {
			try {
				...
				System.out.println("잘못된 메뉴입니다."); //메뉴를 잘못 입력하면 입력 버퍼에 잘못 입력ㄱ된 메뉴가 남아있어서 비워줘야함.
													// 비워주지 않으면 무한 루프 발생.
				scan.nextLine();
			}
		}while(menu != EXIT);
	...
	private void addWord() {
		...
		// 단어장에 추가
		if(vocabulary.addWord(word, partOfString, mean)) {
			System.out.println("단어를 추가했습니다.");
		}else {
			System.out.println("뜻과 단어가 이미 등록된 생태입니다.");
		}
	}
